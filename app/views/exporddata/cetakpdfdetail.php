...
class FPDF_AutoWrapTable extends FPDF {
    ...
    function __construct($data = array(), $options = array()) {
        ...
        parent::__construct();
        $this->data =$data["datajson"];

       
        // foreach ($this->data as $dataitem){
        //     print_r($dataitem->Detail);
        // }
        // die();
    ...
    public function rptDetailData () {
        ...
        $tahun = $dataheader["tahun"];
        $bulan = $this->rubahbulanangka($dataheader["bulan"]);

        $sethead = substr($bulan,0,3).' '.$tahun;
        ...
        $this->SetTitle('ABSEN');
        $this->SetFont("", "B", 10);
        $this->SetX($left); $this->Cell(0, 5, 'ABSEN PT.BMI', 0, 1,'C');
        $this->SetFont("", "", 10);
        $this->SetX($left); $this->Cell(0, 5, $sethead, 0, 1,'C');
        $this->SetFont("", "", 10);
        ...
        $left = $this->GetX();
        $this->SetFont("", "B", 6);
       // $this->Cell(5,$h,'No',1,0,'C',true);
        $this->SetX($left);$this->Cell(10,$h, 'Userid', 1, 0, 'L',true);
        $this->SetX($left += 10); $this->Cell(18,$h, 'Departemen', 1, 0, 'L',true);
        $this->SetX($left += 18);$this->Cell(12,$h, 'NIK', 1, 0, 'L',true);
        $this->SetX($left += 12); $this->Cell(20,$h,'Nama' , 1, 0, 'L',true);
        $this->SetX($left += 20); $this->Cell(15,$h, 'Jabatan', 1, 0, 'L',true);
        $this->SetX($left += 15); $this->Cell(20,$h, 'Salery', 1, 0, 'R',true);
        $this->SetX($left += 20); $this->Cell(12,$h, 'HK', 1, 0, 'R',true);
        $this->SetX($left += 12); $this->Cell(12,$h, 'Hadir', 1, 0, 'R',true);
        $this->SetX($left += 12); $this->Cell(15,$h, 'Jam Lembur', 1, 0, 'R',true);
        $this->SetX($left += 15); $this->Cell(20,$h, 'Gapok', 1, 0, 'R',true);
        $this->SetX($left += 20); $this->Cell(20,$h, 'Gaji Lembur', 1, 0, 'R',true);
        $this->SetX($left += 20); $this->Cell(20,$h, 'Gaji Diterima', 1, 1, 'R',true);
        #data dari database
        $this->SetFont('Arial','',6);
        $this->SetWidths(array(10,18,12,20,15,20,12,12,15,20,20,20));
        $this->SetAligns(array('C','L','L','L','L','R','R','R','R','R','R','R','R'));
			...
			$this->SetX(75);$this->Cell(20,$h,$label,0, 0, 'L',true);
		    $this->SetX(85);$this->Cell(20,$h,$setsaley, 1, 0, 'R',true);
